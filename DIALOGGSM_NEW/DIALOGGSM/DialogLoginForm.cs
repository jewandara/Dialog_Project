...
namespace DIALOGGSM
{
    public partial class DialogLoginForm : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            String KeyData = " @APPKEY = '" + parthKEY + "', @UserNumber = '" + UserID + "' , @UserPass = '" + textBoxPassWord.Text + "'";
            config callServer = new config();
            DataTable dt = callServer.dialogServerInsert("LOGIN_APPLICATION", KeyData);
            ...
            if (dr["SUCESS"].ToString() == "1")
            {
                try
                {
                    this.Hide();
                    this.Visible = false;
                    DialogMainForm MainForm = new DialogMainForm(Int32.Parse(dr["USER_LOGIN_APP_NUMBER"].ToString()), Int32.Parse(dr["USER_OWN_LOGIN_APP_NUMBER"].ToString()), dr["USER_LOGIN_ID"].ToString(), dr["USER_NUMBER"].ToString());
                    MainForm.Show();
                }
                catch (Exception ex)
                {
        ...
        {
            if(textBoxUserName.TextLength == 11){
                labelMessage.Text = "";
                labelMessage.Visible = true;
                labelMessage.ForeColor = Color.White;
